...
Flow:
  ...
  3. Insert CV + embedding into S3 (configured CV bucket)
...
def extract_text_from_pdf(pdf_bytes: bytes) -> str:
    ...
    try:
        ...
        result = "\n".join(text_pages)
        logger.info(
            "PDF extracted  pages=%d  chars=%d", len(pdf_reader.pages), len(result)
        )
        return result
...
def match_cv(
    ...
    logger.info(
        "CV embedding generated  dims=%d  time=%.3fs", len(embedding), embed_elapsed
    )
    ...
    all_job_ids_sorted = sorted(
        best_by_job.keys(), key=lambda j: best_by_job[j], reverse=True
    )
    ...
    if has_filters:
        ...
        if len(strict_matches) < MIN_STRICT:
            ...
            logger.info(
                ...
                len(strict_matches),
                MIN_STRICT,
                min(needed, len(relaxed_matches)),
            )
